import { google } from 'googleapis';
import fs from 'fs';
import path from 'path';

const SPREADSHEET_ID = '17lv6ZVW1k79iEV5xJks_tCEUGXdSIik6ZnJ5SbejCwM';

export default async function handler(req, res) {
  // CORS 처리
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');

  if (req.method === 'OPTIONS') {
    return res.status(200).end();
  }

  if (req.method !== 'POST') {
    return res.status(405).json({ message: 'Method Not Allowed' });
  }

  try {
    const credentials = {
      "type": "service_account",
      "project_id": "profile-499810",
      "private_key_id": "5cd0fbf296bef33ce27d7e4344c6d9f21435de2c",
      "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
      "client_email": "profile@profile-499810.iam.gserviceaccount.com"
    };

    const auth = new google.auth.GoogleAuth({
      credentials,
      scopes: ['https://www.googleapis.com/auth/spreadsheets'],
    });

    const sheets = google.sheets({ version: 'v4', auth });
    
    const { sender, phone, email, title, content } = req.body;
    const timestamp = new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' });

    // 시트1에 데이터 추가
    await sheets.spreadsheets.values.append({
      spreadsheetId: SPREADSHEET_ID,
      range: '\uC2DC\uD2B81!A:F',
      valueInputOption: 'USER_ENTERED',
      requestBody: {
        values: [[timestamp, sender, phone, email, title, content]],
      },
    });

    // 텔레그램 봇 알림 전송
    const TELEGRAM_TOKEN = '8891532723:AAHjoKt_NInTmrArVwVxKax2tNJZVWnCG3Q';
    const TELEGRAM_CHAT_ID = '8687389890';
    
    const telegramMessage = `💡 [새로운 강의 문의 도착!]
▪️ 소속/성함: ${sender}
▪️ 연락처: ${phone}
▪️ 이메일: ${email}
▪️ 제목: ${title}

▪️ 문의 내용:
${content}`;

    await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        chat_id: TELEGRAM_CHAT_ID,
        text: telegramMessage
      })
    });

    res.status(200).json({ success: true, message: 'Successfully saved to Google Sheets and sent Telegram.' });
  } catch (error) {
    console.error('Error appending to Google Sheets or sending Telegram:', error);
    res.status(500).json({ success: false, message: error.toString() });
  }
}
